...
public class MiniMap {
	...
	Texture[] room = new Texture[6];//0 - wall;1-exit;2o - exit; 3 - 3exitl 4 - 4 exit, 5 - 2b - exit
		...
		for(int i = 0; i<GamePlayState.SIZE;i++)
			for(int j = 0; j<GamePlayState.SIZE;j++)
				if(map[i][j]){
					...
					batch.draw(texture, j*50-Chunk.SIZE*50/2, i*50-Chunk.SIZE*50/2, 25, 25, 50, 50, 1, 1, -90*GamePlayState.chunks[i][j].rotates);
				}
	}
